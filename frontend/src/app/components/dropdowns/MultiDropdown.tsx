...
const Dropdown = ({
  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      ...
      >
        <span className="grow">{displayLabel}</span>
        <span
      ...
      <label
        htmlFor={id}
        className={`${value.length > 0 || isOpen ? `-top-4 ${onModal ? "bg-[var(--bg-modal)]" : "bg-[var(--bg-main)]"} font-semibold text-[var(--accent-color)]` : "top-[57.5%] -translate-y-[65%] bg-transparent"} pointer-events-none absolute left-3 z-2 pr-1.5 pl-1.5 transition-[translate,top] duration-[var(--slow)] select-none`}
      >
        {label}
      </label>
      ...
      >
        ...
        {options.map((opt, index) => (
          <li
            ...
            tabIndex={0}
            className={`${value.includes(opt.value) ? "font-bold" : ""} cursor-pointer p-4 transition-colors duration-[var(--slow)] select-none hover:bg-[var(--accent-color)]`}
            role="option"
            onClick={(e) => {
              e.stopPropagation();
