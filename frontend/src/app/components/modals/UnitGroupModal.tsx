...
const UnitGroupModal = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (props.unitGroupId !== null && props.unitGroupId !== undefined) {
      fetchUnitGroup();
    } else {
      setName("");
    }
  }, [props.isOpen, props.unitGroupId]);
  ...
  const fetchUnitGroup = async () => {
    try {
      const response = await fetch(
        `${apiUrl}/unit-group/fetch/${props.unitGroupId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  // --- Update unit group ---
  const updateUnitGroup = async (event: FormEvent, id: number) => {
    event.preventDefault();

    try {
      const response = await fetch(
        `${apiUrl}/unit-group/update/${props.unitGroupId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <>
      {props.isOpen && (
        <ModalBase
          ...
          icon={props.unitGroupId ? PencilSquareIcon : PlusIcon}
          label={
            props.unitGroupId
        ...
        >
          <form
            ...
            onSubmit={(e) =>
              props.unitGroupId
                ? updateUnitGroup(e, props.unitGroupId)
                : addUnitGroup(e)
            }
          >
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:justify-between">
              ...
              >
                {props.unitGroupId ? "Uppdatera" : "Lägg till"}
              </button>
              <button
                type="button"
